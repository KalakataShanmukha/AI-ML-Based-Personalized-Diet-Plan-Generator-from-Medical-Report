...
if process_btn:
    if uploaded_file:
        ...
        <div class='card'>
            ...
            <p style="white-space: pre-line;">{text[:800]}</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        diet_text = f"""
❤️ **Condition**
- {chr(10).join(diet['condition'])}

✅ **Allowed Foods**
- {chr(10).join(diet['allowed'])}

❌ **Restricted Foods**
- {chr(10).join(set(diet['restricted']))}

🥗 **Diet Plan**
- {chr(10).join(diet['diet'])}

🏃 **Lifestyle Advice**
- {chr(10).join(diet['lifestyle'])}
"""
        st.markdown(f"""
        <div class='card'>
            <h4>🍽 Personalized Diet Recommendation</h4>
            <div style="white-space: pre-line;">{diet_text}</div>
        </div>
        """, unsafe_allow_html=True)

        # ---------- JSON DOWNLOAD ----------
        diet_json = json.dumps(diet, ensure_ascii=False, indent=4)
        st.download_button(
            label="📥 Download Diet JSON",
            data=diet_json,
            file_name="diet_recommendation.json",
            mime="application/json"
        )
    else:
        st.warning("⚠️ Please upload a medical report file to generate diet recommendation.")
