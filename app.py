...
with center:
    ...
    st.button("Generate Personalized Diet Plan")
...
if generate_btn:
    ...
    with st.spinner("🔄 Analyzing your medical report..."):
        ...
        # -------- TEXT EXTRACTION --------
        extracted_text, numeric_data = extract_text(uploaded_file)

        with st.expander("📝 Extracted Medical Text"):
            st.write(extracted_text if extracted_text else "No readable text found.")

        # -------- ML HEALTH PREDICTION --------
        health_status = "Unknown"
        if numeric_data:
            health_status = safe_predict_condition(numeric_data)
