...
def generate_diet(text, diet_type="Veg"):
    ...
    meals = {
        "Veg": {
            ...
            "Snacks": ["Fruits", "Nuts", "Sprouts", "Yogurt", "Protein Shake", "Seeds"],
            "Dinner": ["Soup", "Salad", "Grilled Veg", "Tofu", "Paneer", "Zoodles", "Light Curry"]
        },
        "Non-Veg": {
            "Breakfast": ["Oats", "Eggs", "Toast", "Smoothie", "Poha", "Dosa"],
            "Lunch": ["Chicken Curry", "Fish Curry", "Egg Biryani", "Dal Rice", "Quinoa Bowl", "Salad", "Paneer"],
            "Snacks": ["Fruits", "Nuts", "Boiled Eggs", "Protein Shake", "Seeds"],
            "Dinner": ["Grilled Chicken", "Fish", "Egg Curry", "Soup", "Salad", "Tofu", "Light Curry"]
        }
    }
    ...
    df = pd.DataFrame({
        "Day": days,
        "Breakfast": random.sample(meals[diet_type]["Breakfast"], 7),
        "Lunch": random.sample(meals[diet_type]["Lunch"], 7),
        "Snacks": random.sample(meals[diet_type]["Snacks"], 7),
        "Dinner": random.sample(meals[diet_type]["Dinner"], 7)
    })
...
if st.button("🍽 Generate Diet Plan") and uploaded_file:
    ...
    # Generate PDF
    pdf_file = create_pdf(name, age, condition, diet_type, avoid, lifestyle, diet_df)
