...
# -------------------- SMART LIFESTYLE EXTRACTION --------------------
def extract_lifestyle_sentences(text):
    """
    Extract lifestyle-related sentences dynamically from the report.
    """
    sentences = re.split(r'(?<=[.!?]) +', text)  # Split into sentences
    lifestyle_keywords = ["exercise", "sleep", "water", "diet", "stress", "walk", "workout", "physical activity", "healthy", "avoid", "nutrition"]

    extracted = []
    for s in sentences:
        if any(word in s.lower() for word in lifestyle_keywords):
            cleaned = s.strip().capitalize()
...
def generate_diet(text, diet_type="Veg"):
    ...
    if "diabetes" in text:
        ...
        general_meals = {
            ...
            "Non-Veg": {
                "Breakfast": ["Egg omelette","Boiled eggs","Oats with milk","Smoothie with protein","Masala oats with eggs"],
                "Lunch": ["Chicken salad","Grilled fish","Egg curry","Tuna salad","Chicken stir fry"],
                "Snacks": ["Boiled eggs","Chicken slices","Protein shake","Greek yogurt","Nuts"],
                "Dinner": ["Grilled chicken","Fish curry","Egg bhurji","Seafood stir fry","Chicken soup"]
            }
        }
        meals = general_meals[diet_type]

    # -------------------- Generate Lifestyle Advice --------------------
    lifestyle = extract_lifestyle_sentences(text)

    # -------------------- Generate 7-day plan --------------------
    days = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    breakfast_plan = random.sample(meals["Breakfast"], 7)
    lunch_plan = random.sample(meals["Lunch"], 7)
    snacks_plan = random.sample(meals["Snacks"], 7)
    dinner_plan = random.sample(meals["Dinner"], 7)
