...
def generate_diet(text, diet_type="Veg"):
    ...
    cholesterol_meals = {
        "Veg": {
            ...
            "Dinner": ["Stir-fried veggies with tofu","Vegetable soup with herbs","Zoodles with tomato sauce","Steamed vegetables with brown rice","Quinoa bowl with roasted vegetables","Vegetable curry with millet roti","Paneer tikka with green salad","Lentil and spinach soup","Low-fat vegetable stew","Masala grilled paneer with salad","Zucchini and carrot noodles","Vegetable stir fry with quinoa"]
        },
        "Non-Veg": {
    ...
    if diet_type == "Non-Veg":
        if condition == "Diabetes":
            meals = diabetes_meals["Non-Veg"]
        elif condition == "High Cholesterol":
            meals = cholesterol_meals["Non-Veg"]
        else:
            meals = {
                "Breakfast": ["Eggs","Oats with milk","Smoothie with protein","Boiled eggs","Poha with eggs"],
                "Lunch": ["Grilled chicken","Egg curry","Fish curry","Tuna salad","Seafood soup"],
                "Snacks": ["Boiled eggs","Chicken salad","Protein shake","Greek yogurt","Roasted chickpeas"],
                "Dinner": ["Grilled chicken","Egg curry","Seafood stir fry","Baked fish","Chicken soup"]
            }
    else:  # Veg
        if condition == "Diabetes":
            meals = diabetes_meals["Veg"]
        elif condition == "High Cholesterol":
            meals = cholesterol_meals["Veg"]
        else:
            meals = {
...
if st.button("🍽 Generate Diet Plan") and uploaded_file:
    ...
    st.download_button(
        ...
        json.dumps(diet_df.to_dict(orient="records"), indent=2),
        "AI_Diet_Plan.json",
