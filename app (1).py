...
if process_btn:
    ...
    st.markdown("""
    <div class="card">
    <h4 class="title">📄 Extracted Text</h4>
    <p>{}</p>
    </div>
    """.format(text[:1000]), unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="card">
    <h4 class="title">🍽️ Personalized Diet Recommendation</h4>

    <p><b>❤️ Condition:</b> {condition}</p>
    <p><b>✅ Allowed Foods:</b> {allowed}</p>
    <p><b>❌ Restricted Foods:</b> {restricted}</p>
    <p><b>🥗 Diet Plan:</b> {plan}</p>
    <p><b>🚶 Lifestyle Advice:</b> {advice}</p>

    </div>
    """.format(
        condition=diet["condition"],
        allowed=", ".join(diet["allowed_foods"]),
        restricted=", ".join(diet["restricted_foods"]) if diet["restricted_foods"] else "None",
        plan=diet["diet_plan"],
        advice=diet["lifestyle_advice"]
    ), unsafe_allow_html=True)
