...
def generate_diet(text):
    ...
    for i in range(7):
        rows.append({
            "Day": days_of_week[i],
            "Breakfast": f"{emoji_map['Breakfast']} {breakfast_list[i]}",
            "Lunch": f"{emoji_map['Lunch']} {lunch_list[i]}",
            "Snacks": f"{emoji_map['Snacks']} {snacks_list[i]}",
            "Dinner": f"{emoji_map['Dinner']} {dinner_list[i]}"
        })
